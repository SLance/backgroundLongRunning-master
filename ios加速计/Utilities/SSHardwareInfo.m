...
+ (NSDate *)systemUptime2 {
    ...
    NSDate *Date = [[NSDate alloc] initWithTimeIntervalSinceNow:(0-UptimeInterval)];
       NSLog(@"uptimeinterval = %@",Date);
    return [self getNowDateFromatAnDate:Date] ;
}

+ (int )systemSecondsMicroTimeToNow
{
    ...
    NSDateComponents *Components = [Calendar components:unitFlags fromDate:Date toDate:[self getNowDateFromatAnDate:[NSDate date]]  options:0];
    ...
    // Format the dates
    return [Days intValue]*24*60*60   +   [Hours intValue]*60*60   + [Minutes intValue]*60;
}
...
+ (NSString *)systemUptime {
    ...
    NSCalendar *Calendar = [NSCalendar currentCalendar];
 	// Create the Dates
    NSDate *Date = [self getNowDateFromatAnDate:[[NSDate alloc] initWithTimeIntervalSinceNow:(0-UptimeInterval)]];
...
+ (NSString *)systemDeviceType:(BOOL)formatted {
